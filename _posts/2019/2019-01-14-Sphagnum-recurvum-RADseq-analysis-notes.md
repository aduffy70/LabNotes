...
# More ipyrad gotchas
  ...
  * Step 6 on 384 samples takes a long time (after 28hours it was only 23%) on the building database step. Apparently this part has to run in serial so throwing multiple nodes at it doesn't help, but it is also memory hungry so raising the memory limits will. I am trying step 6 on just 2 cpus but with 46GB of memory to see how it changes.
